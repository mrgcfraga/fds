...
USE :: RADCONS, ONLY: WEIGHT_RTE_SOURCE_CORRECTION
INTEGER :: IBND
REAL(EB), PARAMETER :: WGT
REAL(EB) :: RAD_Q_SUM_ALL,KFST4_SUM_ALL,TNOW

TNOW = CURRENT_TIME()

WGT = WEIGHT_RTE_SOURCE_CORRECTION

DO IBND=1,NUMBER_SPECTRAL_BANDS

	! Sum up the components of the corrective factor from all the meshes.
	IF (N_MPI_PROCESSES>1) THEN
		CALL MPI_ALLREDUCE(RAD_Q_SUM(IBND),RAD_Q_SUM_ALL,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,IERR)
		CALL MPI_ALLREDUCE(KFST4_SUM(IBND),KFST4_SUM_ALL,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,IERR)
	ELSE
		RAD_Q_SUM_ALL = RAD_Q_SUM(IBND)
		KFST4_SUM_ALL = KFST4_SUM(IBND)
	ENDIF

	! Compute the corrective factor for the RTE. Note that the max value of 100 is arbitrary.
	
	IF (KFST4_SUM_ALL>TWO_EPSILON_EB) &
		RTE_SOURCE_CORRECTION_FACTOR(IBND) = WGT*RTE_SOURCE_CORRECTION_FACTOR(IBND) + &
			(1._EB-WGT)*MIN(C_MAX,MAX(C_MIN,RAD_Q_SUM_ALL/(KFST4_SUM_ALL+TWO_EPSILON_EB)))
