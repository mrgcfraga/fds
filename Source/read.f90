...
REWIND(LU_INPUT) ; INPUT_FILE_LINE_NUMBER = 0
COUNT_MOVE_LOOP: DO
   CALL CHECKREAD('MOVE',LU_INPUT,IOS)
...
NAMELIST /DUMP/ CFL_FILE,CLIP_RESTART_FILES,COLUMN_DUMP_LIMIT,CTRL_COLUMN_LIMIT,&
                ...
                FLUSH_FILE_BUFFERS,GEOM_DIAG,MASS_FILE,MAXIMUM_PARTICLES,MMS_TIMER,NFRAMES,PLOT3D_PART_ID, &
                PLOT3D_QUANTITY,PLOT3D_SPEC_ID,PLOT3D_VELO_INDEX,RENDER_FILE,SIG_FIGS,SIG_FIGS_EXP,SMOKE3D, &
                SMOKE3D_QUANTITY,SMOKE3D_SPEC_ID,STATUS_FILES,SUPPRESS_DIAGNOSTICS,T_DEVC_LINE_END,T_RADF_BEGIN,T_RADF_END,&
                UVW_TIMER,VELOCITY_ERROR_FILE,WRITE_XYZ,GET_CUTCELLS_VERBOSE
...
DO N = 1, N_TRACKED_SPECIES
   ...
   IF (SPECIES_MIXTURE(N)%SINGLE_SPEC_INDEX==CO2_INDEX) NU_SOOT_OX(N) = SS%ATOMS(1) * 0.5_EB * MW_H2O / SS%MW
   IF (SPECIES_MIXTURE(N)%SINGLE_SPEC_INDEX==H2O_INDEX) NU_SOOT_OX(N) = SS%ATOMS(6) * MW_CO2 / SS%MW
END DO
...
NAMELIST /COMB/ CHECK_REALIZABILITY,EXTINCTION_MODEL,FIXED_MIX_TIME,&
...
REWIND(LU_INPUT) ; INPUT_FILE_LINE_NUMBER = 0
COMB_LOOP: DO
   CALL CHECKREAD('COMB',LU_INPUT,IOS)
...
CHARACTER(LABEL_LENGTH) :: FUEL,RADCAL_ID='null',SPEC_ID_NU(MAX_SPECIES),SPEC_ID_N_S(MAX_SPECIES),RAMP_AIT,RAMP_CHI_R,FORMULA
...
REAL(EB) :: SOOT_YIELD,CO_YIELD,EPUMO2,A,LOWER_OXYGEN_LIMIT, &
            ...
            AUTO_IGNITION_TEMPERATURE,SOOT_H_FRACTION,N_T,K,NU(MAX_SPECIES),N_S(MAX_SPECIES),FRAC
...
NAMELIST /REAC/ A,AUTO_IGNITION_TEMPERATURE,C,CHECK_ATOM_BALANCE,CO_YIELD,CRITICAL_FLAME_TEMPERATURE,&
                ...
                RAMP_AIT,RAMP_CHI_R,REAC_ATOM_ERROR,REAC_MASS_ERROR,REVERSE,SOOT_H_FRACTION,SOOT_YIELD,&
...
REAC_READ_LOOP: DO NR=1,N_REACTIONS
   ...
   IF (NR==2 .AND. N_SIMPLE_CHEMISTRY_REACTIONS==2) THEN
      ...
      PRIORITY = 2
      AUTO_IGNITION_TEMPERATURE = 605._EB
   ELSE
      CALL CHECKREAD('REAC',LU_INPUT,IOS)
   ...
   IF (EXTINCT_MOD==EXTINCTION_1) THEN
      FRAC = MIN(0.01_EB,CHARACTERISTIC_CELL_SIZE)/CHARACTERISTIC_CELL_SIZE
      EXTINCTION_1_TMP_CUTOFF = FRAC*RN%CRIT_FLAME_TMP + (1._EB-FRAC)*873._EB
   ENDIF
   RN%E                         = E*1000._EB
   RN%E_IN                      = E
   RN%K                         = K
   ...
   IF (RN%RAMP_AIT/='null') CALL GET_RAMP_INDEX(RN%RAMP_AIT,'TIME',RN%RAMP_AIT_INDEX)
...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2),&
            ...
            AUTO_IGNITION_TEMPERATURE,AREA_ADJUST,PARTICLE_SURFACE_DENSITY,&
            FSK_K(MAX_NUMBER_FSK_POINTS),FSK_A(MAX_NUMBER_FSK_POINTS),FSK_W(MAX_NUMBER_FSK_POINTS)
...
NAMELIST /SURF/ ADIABATIC,AREA_ADJUST,AUTO_IGNITION_TEMPERATURE,&
                ...
                NO_SLIP,NPPC,PARTICLE_MASS_FLUX,PARTICLE_SURFACE_DENSITY,PART_ID,PLE,PROFILE,RADIUS,RAMP_EF,RAMP_MF,&
                RAMP_PART,RAMP_Q,RAMP_T,RAMP_T_I,RAMP_V,RAMP_V_X,RAMP_V_Y,RAMP_V_Z,RGB,ROUGHNESS,SPEC_ID,&
                SPREAD_RATE,STRETCH_FACTOR,&
...
NAMELIST /RADI/ ANGLE_INCREMENT,BAND_LIMITS,C_MAX,C_MIN,INITIAL_RADIATION_ITERATIONS,KAPPA0,NMIEANG,NUMBER_RADIATION_ANGLES,&
                ...
                TIME_STEP_INCREMENT,WIDE_BAND_MODEL,MIE_MINIMUM_DIAMETER,MIE_MAXIMUM_DIAMETER,MIE_NDG,WSGG_MODEL,&
                SOOT_DENSITY,WEIGHT_RTE_SOURCE_CORRECTION,C_MAX,NUMBER_INITIAL_ITERATIONS ! Backward compatibility
...
WSGG_MODEL				= .FALSE.
C_MAX						= 100._EB
WEIGHT_RTE_SOURCE_CORRECTION = 0.5_EB
...
NAMELIST /INIT/ AUTO_IGNITION_TEMPERATURE,CELL_CENTERED,CTRL_ID,DENSITY,DEVC_ID,DIAMETER,DT_INSERT,DX,DY,DZ,&
                ...
                RADIUS,SHAPE,SPEC_ID,TEMPERATURE,UNIFORM,UVW,VOLUME_FRACTION,XB,XYZ,&
...
READ_DEVC_LOOP: DO NN=1,N_DEVC_READ
   ...
   POINTS_LOOP: DO I_POINT=1,POINTS
      ...
      IF (DV%SPATIAL_STATISTIC=='MIN') MIN_DEVICES_EXIST = .TRUE.
      IF (DV%SPATIAL_STATISTIC=='MAX') MAX_DEVICES_EXIST = .TRUE.
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   REWIND(LU_INPUT) ; INPUT_FILE_LINE_NUMBER = 0
   COUNT_RADF_LOOP: DO
      CALL CHECKREAD('RADF',LU_INPUT,IOS)
...
SUBROUTINE GET_QUANTITY_INDEX(SMOKEVIEW_LABEL,SMOKEVIEW_BAR_LABEL,OUTPUT_INDEX,OUTPUT2_INDEX, &
                              ...
                              QUANTITY,QUANTITY2,SPEC_ID_IN,PART_ID,DUCT_ID,NODE_ID,REAC_ID,MATL_ID,CELL_L,DUCT_CELL_INDEX)
